...
BEGIN
    ...
    BEGIN TRY
        ...
        SELECT
            ...
            (
                CASE WHEN @v_CleanSearch <> '' AND LOWER(P.ProductName + ' ' + P.ProductVariableName) LIKE '%' + @v_CleanSearch + '%' THEN 30 ELSE 0 END +
                ...
                CASE WHEN @v_CantidadTerminosReales > 0 THEN
                    (SELECT COUNT(*) * 10 FROM @TerminosUsuario T WHERE LOWER(P.ProductName + ' ' + P.ProductVariableName) LIKE '%' + T.Termino + '%')
                ELSE 0 END
            ) AS MatchScore
        FROM [DB_ECOMMERCE].[SQM_GENERAL].[VW_GENERAL_PRODUCTS] P (NOLOCK)
        WHERE P.StockAvilable > 0;

        -- [!] SE MODIFICÓ LA DECLARACIÓN: Se añadió la columna MatchScore
        ...
        -- ===================================================================
        -- [!] CORRECCIÓN DE EVALUACIÓN DE AMBIGÜEDAD
        -- ===================================================================
        -- 1. Obtener cuál fue el puntaje máximo alcanzado
        DECLARE @MaxScore INT = (SELECT MAX(MatchScore) FROM @TmpVariantes);

        -- 2. Limpiar de la tabla los productos que no alcanzaron el puntaje máximo
        DELETE FROM @TmpVariantes WHERE MatchScore < @MaxScore;

        -- 3. Ahora sí, contar si hay un empate en el primer lugar
